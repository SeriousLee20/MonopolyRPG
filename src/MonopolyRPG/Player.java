...
public class Player extends Role implements Comparable<Player>{
    ...
    public void levelUp(){
        ...
        this.level ++;  // +1 level
        weaponBag.upgrade(level, this.name);   // when up lvl, then upgrade weapon bag
    }
    ...
    // required some more EXP to level up
    private int getEXP_required(){
        return getEXP_nextLevel() - this.currentEXP;
    }

    // get player's location on the board
    public int getLocation() {
        return location;
    }

    // record player's location on the board
    ...
    public void useItem(Item item){
        ...
        if (item.getSpecialEffect() == Item.SpecialEffect.ONE_HUNDRED_PERCENT_RUN) {
            ...
            // potion +20 current HP, Hi-potion +50 current HP
        }
    }
    ...
    @Override
    // determine winner based on level
    // if same level then based on gold
    ...
    // assign index to player based on game sequence
