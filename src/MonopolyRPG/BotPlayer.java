...
public class BotPlayer extends Player{
    ...
    //select item based on gold remained
    public Item selectItemToBuy(){
        if(this.getGold() > Shop.HIPOTION_PRICE)
            return Item.HIPOTION;
        else if(this.getGold() > Shop.SMOKEBOMB_PRICE)
            return Item.SMOKEBOMB;
        else
            return Item.POTION;
    }

    // select weapon to buy based on level, if owned then choose other
