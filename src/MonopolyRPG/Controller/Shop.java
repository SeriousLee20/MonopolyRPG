...
public class Shop {
    ...
    public final static int POTION_PRICE = 29, SMOKEBOMB_PRICE = 30, HIPOTION_PRICE = 100;
    public final static int AXE_PRICE = 20, SPEAR_PRICE = 40, HAMMER_PRICE = 35;
    ...
    public void promptBuy(Player player, int shop){
        ...
        if(shop == 1){
            ...
            if (player instanceof BotPlayer) {
                ...
                System.out.println("Which weapon you want to buy: " + weapon);
                if (player.getWeaponBag().ownWeapon(weapon)) {
                    return;
                }
            } else {

                System.out.print("Which weapons you want to buy: ");
    ...
    public void buyItem(Player player, Item item, int amount, int price){
        ...
        player.obtainItem(item,amount);     //item is added to the item bag
    ...
    public void buyWeapon(Player player, Weapon weapon, int price) {
        ...
        player.obtainWeapon(weapon);    // weapon is added to the weapon bag
        player.spendGold(price);        // amount of gold is deducted
        System.out.println("It costs " + price + "\nRemaining gold: " + player.getGold() + "\n");
        System.out.println("-+-".repeat(25));
    }
    ...
    public void promptSell(Player player){
        ...
        ItemBag bag = player.getItemBag();  //getItemBag return ItemBag type
        Item item = bag.get(itemID);    // search item from bag
